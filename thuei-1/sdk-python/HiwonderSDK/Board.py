...
def setMotor(index, speed):
    ...
    index = __motor_init_map[index-1]
    index = index - 1
    speed = 100 if speed > 100 else speed
    speed = -100 if speed < -100 else speed
    speed = speed*__motor_init_dir[index]
...
def initMotors(motor_init_dir, motor_init_map = [1, 2, 3, 4]):
    # check if motor_init_dir is a list consisting of 4 elements which is 1 or -1
    if (not isinstance(motor_init_dir, list) 
        or len(motor_init_dir) != 4 
        or not all([x in [-1, 1] for x in motor_init_dir])):
        raise AttributeError("Invalid motor_init_dir: %s, should be like [1,-1,-1,1], etc."%
                             str(motor_init_dir))
    # check if motor_init_dir is a list consisting of 4 elements which is 1 or 2 or 3 or 4
    if (not isinstance(motor_init_map, list) 
        or len(motor_init_map) != 4 
        or not all([x in [1, 2, 3, 4] for x in motor_init_map])):
        raise AttributeError("Invalid motor_init_map: %s, should be like [2,1,4,3], etc."%
                             str(motor_init_map))
    global __motor_init_dir, __motor_init_map
    __motor_init_dir = motor_init_dir
    __motor_init_map = motor_init_map
